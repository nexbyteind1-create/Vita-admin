...
const columns: Column<Admin>[] = [
  { key: "name", header: "Admin", sortable: true, render: a => (
    <div className="flex items-center gap-3">
      <div className="w-8 h-8 bg-gradient-to-br from-purple-600/40 to-purple-900/40 rounded-full flex items-center justify-center text-xs font-bold text-purple-300 border border-purple-500/20">
        {a.name.split(" ").map(n => n[0]).join("").slice(0, 2)}
      </div>
      <div><div className="text-sm font-semibold text-slate-200">{a.name}</div><div className="text-xs text-slate-500">{a.email}</div></div>
    </div>
  )},
  ...
  { key: "permissions", header: "Permissions", render: a => <span className="text-slate-400 text-xs">{a.permissions.length} assigned</span> },
  { key: "status", header: "Status", render: a => <Badge variant={a.status} /> },
  { key: "lastLogin", header: "Last Login", render: a => <span className="text-slate-400 text-sm">{a.lastLogin ? formatDate(a.lastLogin) : "Never"}</span> },
];
          ...
          {selected && <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {selected.permissions.map(p => <span key={p} className="text-xs bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-full px-2.5 py-1">{p.replace(/_/g, " ")}</span>)}
              </div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div><label className="text-xs font-semibold text-slate-400 uppercase mb-2 block">Permissions</label>
              <div className="grid grid-cols-2 gap-2">
                {allPermissions.map(p => <label key={p} className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer"><input type="checkbox" className="accent-blue-500" />{p.replace(/_/g, " ")}</label>)}
              </div>
            </div>
          </div>
          <div className="flex gap-2 pt-4 border-t border-[#1f2d45] mt-4">
