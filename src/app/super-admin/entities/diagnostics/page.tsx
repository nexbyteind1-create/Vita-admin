...
const columns: Column<DiagnosticCenter>[] = [
  { key: "name", header: "Center", sortable: true, render: d => <div><div className="text-sm font-semibold text-slate-200">{d.name}</div><div className="text-xs text-slate-500">{d.licenseNumber}</div></div> },
  { key: "city", header: "Location", render: d => <span className="text-slate-400 text-sm">{d.city}, {d.state}</span> },
  { key: "totalBookings", header: "Bookings", align: "right", render: d => <span className="font-semibold">{formatNumber(d.totalBookings)}</span> },
  { key: "pendingReports", header: "Pending", align: "center", render: d => <span className={`font-semibold ${d.pendingReports > 40 ? "text-red-400" : "text-amber-400"}`}>{d.pendingReports}</span> },
