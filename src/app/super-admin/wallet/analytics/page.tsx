  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="p-6 space-y-6 max-w-[1600px]">
        ...
        {/* Top Earners */}
        <section>
          <h2 className="text-base font-bold text-white mb-4">Top 5 Credit Earners</h2>
            ...
            <table className="vita-table w-full">
              ...
              <tbody>
                {topCreditEarners.map(e => (
                  <tr key={e.rank}>
                    <td><div className="w-7 h-7 bg-gradient-to-br from-amber-500/30 to-amber-700/30 rounded-full flex items-center justify-center text-xs font-bold text-amber-400">{e.rank}</div></td>
                    <td className="font-medium text-slate-200">{e.name}</td>
                    <td className="text-slate-400">{e.uhid}</td>
                    <td><span className={`text-xs font-semibold ${e.plan === "Platinum" ? "text-purple-400" : e.plan === "Gold" ? "text-amber-400" : "text-slate-400"}`}>{e.plan}</span></td>
                    <td className="text-right font-bold text-emerald-400">{formatNumber(e.totalCredits)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
