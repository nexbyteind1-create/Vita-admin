    ...
    <div className="glass-card px-4 py-3 border border-[#1f2d45] text-sm">
      ...
      {payload.map(p => (
        <div key={p.name} className="flex items-center gap-2">
          ...
          <span className="font-semibold text-white">{formatValue ? formatValue(p.value) : p.value.toLocaleString("en-IN")}</span>
        </div>
      ))}
    </div>
  );
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <RechartsArea data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
          ...
          <YAxis tick={{ fill: "#64748b", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={v => formatValue ? formatValue(v) : v >= 1000 ? `${(v/1000).toFixed(0)}K` : String(v)} width={45} />
          <Tooltip content={<CustomTooltip formatValue={formatValue} />} />
          {value2Label && <Legend wrapperStyle={{ fontSize: "12px", color: "#94a3b8" }} />}
