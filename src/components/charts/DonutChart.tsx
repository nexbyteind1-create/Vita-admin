  ...
  return (
    <div className="glass-card px-4 py-3 border border-[#1f2d45] text-sm">
      <div className="flex items-center gap-2">
        ...
        <span className="font-bold text-white">{formatValue ? formatValue(payload[0].value) : payload[0].value.toLocaleString("en-IN")}</span>
      </div>
    </div>
  );
};
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <PieChart>
          ...
          <Legend iconType="circle" iconSize={8} formatter={(value) => <span style={{ color: "#94a3b8", fontSize: "12px" }}>{value}</span>} />
        </PieChart>
      </ResponsiveContainer>
    </div>
  );
